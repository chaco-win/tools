...
# Disable Windows Copilot at Startup
...
# --------- PART X: Schedule Memory Test and Forced Reboot at 1 AM ---------
...
# Register task with both actions
Register-ScheduledTask -TaskName $taskName -Action @($action1, $action2) -Trigger $trigger \
    -Description "Run memory diagnostic then reboot at 1 AM" -User "SYSTEM" -RunLevel Highest -Force

Write-Host "Scheduled memory diagnostic and forced reboot at 1:00 AM tonight." -ForegroundColor Green
...
Write-Host "`n=== Checking for Any Available Updates ===" -ForegroundColor Cyan
Import-Module PSWindowsUpdate

# Fetch all updates (no reboot yet)
$allUpdates = Get-WindowsUpdate -AcceptAll -IgnoreReboot
...
if ($allUpdates.Count -gt 0) {
    ...
    Write-Host "All updates installed.  No reboot yet." -ForegroundColor Green
} else {
...
# Exit Prompt with Auto-Close and Abort
Write-Host "`n========================================" -ForegroundColor Blue
Write-Host "===== Script Complete ====="            -ForegroundColor Blue
Write-Host "========================================`n" -ForegroundColor Blue
Write-Host "The script will close automatically in 60 seconds." -ForegroundColor Cyan

$timeout = 60

Write-Host "`nTime's up. Exiting script." -ForegroundColor Yellow
